...
import { FiX, FiEdit3, FiChevronDown } from "react-icons/fi";
import api from "../../services/api";
import type { Policy, PolicyForm, ValidationErrors } from "../../types/policy";
...
export default function EditPolicyModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.put(`/policies/${policy?.id}`, formData);
        ...
        <div className="flex items-start justify-between border-b border-[#12203A]/8 px-6 py-5 sm:px-8 sm:py-6">
          <div className="flex items-center gap-3.5">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#C9A15D]/30 bg-[#C9A15D]/10">
              <FiEdit3 className="h-[18px] w-[18px] text-[#8A6A2F]" />
            </span>
            <div>
              ...
              <p className="mt-0.5 text-[13px] text-[#6B7690]">
                Update the details of this policy
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  {["Life", "Health", "Vehicle", "Home", "Travel"].map((type) => (
